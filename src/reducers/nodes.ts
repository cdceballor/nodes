...
export const checkNodeBlocks = createAsyncThunk(
  ...
  async (node: string) => {
    const response = await fetch(`${node}/api/v1/blocks`);
...
export const checkNodesBlocks = createAsyncThunk(
  ...
  async (nodes: Node[], thunkAPI) => {
    const { dispatch } = thunkAPI;
    nodes.forEach((node) => {
      dispatch(checkNodeBlocks(node.url));
    });
  }
);
...
export const nodesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(checkNodeBlocks.pending, (state, action) => {
      const node = state.list.find((n) => n.url === action.meta.arg);
      if (node) node.loading = true;
    });
...
export const requestNodes = (state: RootState) => state.requests.data;
